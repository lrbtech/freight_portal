                  ...
                  <div class="card-body">
                    <div class="table-responsive">
                      <table class="display" id="basic-1">
                        ...
                        <tbody>
                        @foreach($truck_owner as $key => $row)
                        <tr>
                            <td>{{$key + 1}}</td>
                            <td>
                            <a href="/admin/assign-truck/{{$row->id}}">{{$row->name}}</a>
                            </td>
                            <td>
                            <a href="/admin/assign-truck/{{$row->id}}">{{$row->company_name}}</a>
                            </td>
                            <td>{{$row->mobile}}</td>
                            <td>
                            @if($row->status == 0)
...
<div class="modal fade" id="popup_modal"  tabindex="-1" role="dialog" aria-labelledby="popup_modal" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <div class="form-group">
            ...
            <input autocomplete="off" type="text" id="company_name" name="company_name" class="form-control">
        </div>
        ...
        <div class="form-group">
            ...
            <textarea autocomplete="off" id="address" name="address" class="form-control"></textarea>
        </div>
...
$('#add_new').click(function(){
    ...
    $('#modal-title').text('Add Truck Owner');
});
function Save(){
  var formData = new FormData($('#form')[0]);
  if(action_type == 1){
    $.ajax({
        url : '/admin/save-truck',
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function Edit(id){
  $.ajax({
    url : '/admin/edit-truck/'+id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      $('#modal-title').text('Update Truck');
...
function Delete(id,status){
    ...
    if (r == true) {
      $.ajax({
        url : '/admin/truck-delete/'+id+'/'+status,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
